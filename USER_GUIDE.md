...
| 功能模块 | 说明 | 图标 |
|---------|------|------|
| 地图可视化 | SVG 渲染的交互式路网地图 (1600x800px) | 🗺️ |
| 动态速度调整 | 基于占用率的拥堵减速模拟 | 🚦📉 |
| 仿真控制 | 启动/暂停/重置仿真 | ▶️⏸️🔄 |
| 交通流管理 | 创建和管理车辆流 | 🚗 |
| 性能监控 | 实时 KPI 指标展示 | 📊 |
...
#### 道路状态 (动态速度调整)
-  **拥堵影响车速** - 道路占用率越高，车辆速度越慢
  - 占用率 > 90%: 速度降至 30% (严重拥堵，红色)
  - 占用率 > 75%: 速度降至 50% (高拥堵，橙色)
  - 占用率 > 50%: 速度降至 75% (中等拥堵，黄色)
  - 占用率 > 25%: 速度降至 90% (轻微拥堵)
  - 占用率 < 25%: 正常速度 (畅通，绿色)
